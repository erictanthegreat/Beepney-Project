...
// ------------------- Tricycle Fare Calculation Function -------------------

const calculateTricycleFare = (distanceInKm) => {
  ...
  const SEATS = 4; // If the distance is 0 or less, return 0 fare

  if (distanceInKm <= 0) {
    return 0;
  } // If the distance is within the base distance (0-4 km), return the base fare

  if (distanceInKm <= BASE_DISTANCE_KM) {
    return BASE_FARE_PESOS;
  } // Calculate the distance beyond the base distance

  const additionalDistance = distanceInKm - BASE_DISTANCE_KM; // Calculate the cost for the additional distance, rounding up to the next full kilometer

  const additionalFare = Math.ceil(additionalDistance) * SUCCEEDING_FARE_PER_KM; // Calculate the total fare

  const totalFare = BASE_FARE_PESOS + additionalFare; // Calculate if the user selected solo

  const soloFare = BASE_FARE_PESOS * SEATS; // Calculate if the user selected group

  const groupFare = BASE_FARE_PESOS * SEATS + GROUP_FARE_ADDITIONAL; // Return the total fare, rounded to two decimal places
...
export default function RideHailing() {
  ...
  const [fare, setFare] = useState<number>(0); // New state for fare
  ...
  const fetchRoute = async (start: [number, number], end: [number, number]) => {
    try {
      ...
      if (data.routes && data.routes.length > 0) {
        ...
        setETA(Math.ceil(route.duration / 60)); // Calculate and set the fare based on the calculated distance

        const calculatedFare = calculateTricycleFare(calculatedDistance);
  ...
  }, [pickup, destination]); // --- Fetch address using Mapbox Geocoding API ---
  ...
  return (
    <View style={rideStyles.container}>
      ...
      {/* Header */}
      <View>
        <BackButton /> <Text style={rideStyles.header}>TricyCall</Text>
      ...
      <BottomSheetContainer ref={bottomSheetRef}>
        <View style={rideStyles.bsCont}>
          <Text style={rideStyles.label}>Your Trip</Text> {/* Pickup info */}
          <TouchableOpacity
          ...
          <View style={rideStyles.line}></View> {/* Ride selection */}
          ...
          <View style={rideStyles.line}></View> {/* Route summary */}
          <View style={rideStyles.iconCont}>
            ...
            <View style={rideStyles.iconWithText}>
              <ETAIcon /> <Text style={rideStyles.text2}>{eta} min</Text>
            </View>
            <View style={rideStyles.iconWithText}>
              <FareIcon />
